...
%chunk and zzz refer to field names, e.g pre1, rtr respectively
    ...
    for i = 2:1:size(electrodes, 2)
           if task_id < i
                ...
                analysis.misses.std = std(misses)/sqrt(hitlength(2)-1);
                simple_test = abs(mean(hits) - mean(misses)) - sqrt((analysis.misses.std)^2 + (analysis.misses.std)^2);
                
                if simple_test > 0
                    save(sprintf('%ssuccess_%d%d_%s%s_%s.mat', success_folder, electrodes(i), electrodes(task_id), chunk, zzz, filename), 'analysis')
                else
                    save(sprintf('%sfail_%d%d_%s%s_%s.mat', fail_folder, electrodes(i), electrodes(task_id), chunk, zzz, filename), 'analysis')
                end
           end
