# -*- coding: utf-8 -*-
"""
Created on Thu Apr 28 13:04:26 2022
...
metric = pd.DataFrame(columns = ['value', 'drowsiness'])
metric1 = pd.DataFrame(columns = ['value', 'drowsiness'])
metric2 = pd.DataFrame(columns = ['value', 'drowsiness'])
metric3 = pd.DataFrame(columns = ['value', 'drowsiness'])
...
for i in range(0,2):
    ...
    rough_analysis_performance = data_forLDA['performance']
    #rough_analysis_performance = rough_analysis_performance.replace(['hit', 'miss'], [1, -1])
    rough_analysis_performance = rough_analysis_performance.replace(['hit', 'miss'], [1, -1])
    df_list[i]['value'] = np.multiply(decision, rough_analysis_performance)
    df_list[i] = df_list[i].fillna(f'{drowsy_level}')
    metric = pd.concat([metric, df_list[i]])
ax = sns.violinplot(data = metric, x='value', y ='drowsiness').set(title = f'predictive strength from atoms at {chunk}', xlim = (-1,1))
