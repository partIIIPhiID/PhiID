    ...
    for chunk in chunks:
        for drowsy_level in drowsy_levels:
            for n_i in range(0,499):
                ...
                #print(difference)
                #print(std)
                temp_data = pd.DataFrame([[norm_difference, std, stat_test, f'{elecA}', f'{elecB}', f'{drowsy_level}', f'{distance}', f'{chunk}',f'{atom}']], columns = ['normalized difference', 'std', '-log(p)', 'electrode A', 'electrode B', 'drowsiness', 'seperation', 'chunk', 'atom'])
                data = pd.concat([data, temp_data])
        
        analysis = pd.DataFrame(data.loc[(data['atom'] == f'{atom}') & (data['chunk'] == f'{chunk}')], columns = ['normalized difference', 'std', '-log(p)', 'electrode A', 'electrode B', 'drowsiness', 'seperation'])
        ...
        plt.savefig(f'performance_{atom}_{chunk}_crosselec_crosspart_exp1.png', dpi=300, bbox_inches = "tight")
        plt.clf()
